...
**Phase 2 (Win/Loss Synthesizer)** loads historical deals, runs Welch's t-tests and Cohen's d on 29 behavioral signals, trains a gradient boosting model (AUC 0.841), and derives data-grounded thresholds for four stall signatures: Champion Collapse, Competitor Displacement, Ghost Stall, and Exec Vacuum. It then cross-references each lost deal's behavioral fingerprint against its stated loss reason and flags the 28.1% of deals where the rep's diagnosis doesn't match the evidence. The most common contradiction: "Price / Budget" logged when the deal shows unmistakable Champion Collapse or Exec Vacuum patterns.

**Phase 3 (Stall Predictor)** scores each active deal against the signature library, applies urgency multipliers based on position in the sales cycle, enforces stage gates (Exec Vacuum doesn't fire on Prospecting deals — that's normal, not a stall), and generates a priority-sorted intervention queue. Classification accuracy against ground truth labels: 69.5%.
